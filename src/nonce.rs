...
///
/// Once expired, a nonce will not be used for new rolls.
const EXPIRE_AFTER: Duration = Duration::from_secs(60);

/// A nonce is revealed this long after _expiration_.
pub const REVEAL_AFTER: Duration = Duration::from_secs(60);
...
///
/// 5. After the active nonce expires, spawn a task to reveal the nonce (and trigger payouts) after
...
pub async fn manage_nonces(
    ...
    keys: nostr::Keys,
    db: Db,
    trigger_payouts_tx: mpsc::Sender<db::Round>,
) -> Result<()> {
    ...
    loop {
        let active_nonce = Nonce::new(thread_rng());
        ...
        if let Err(e) = set_active_nonce(
            ...
            // Panicking is rough, but I'm not sure it is safe to proceed without updating the
            // active nonce.
            panic!("Failed to set active nonce after publishing commitment: {e:#}");
        }
...
impl Nonce {
    ...
    fn expire_at(&self) -> Instant {
        self.created_at + EXPIRE_AFTER
    }

    fn reveal_at(&self) -> Instant {
        self.created_at + EXPIRE_AFTER + REVEAL_AFTER
    }
}
