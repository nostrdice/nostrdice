        ...
        match &zap {
            Zap {
                ...
                if roll >= threshold {
                    ...
                    // major nostr clients.
                    #[allow(deprecated)]
                        if let Err(e) = client.send_direct_msg(*roller, format!("You lost. You rolled {roll}, which was bigger than {threshold}. Try again!"), None).await {
                    ...
                    if let Err(e) = upsert_zap(&db, invoice.payment_hash().to_string(), zap) {
                        tracing::error!(%roller, "Failed to set BetState to Loser. Error: {e:#}");
                    }
            ...
            Zap {
                ...
                tracing::debug!(?bet_state, "Skipping {roller} based on bet state");
            }
        }
    }
